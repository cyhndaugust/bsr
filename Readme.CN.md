...
- **参数**: `[directory]` - 可选。要添加或对比的目录路径。如果不指定（且待比对区为空），默认为当前目录 `.`。
- **选项**:
  - `-C, --context <n>`: 设置 Diff 输出显示的上下文行数（默认为 3）。
  - `-os, --originSource <subpath>`: 递归对比 `originSource` 下的特定子目录。需要待比对区已有两个目录。
- **功能**:
  - **待比对区管理**:
    - 支持在“待比对区”中存储最多 **两个** 目录。
    - 当区域已满时添加新目录，会提示替换第 1 个或第 2 个目录，或清空并重新开始。
    - 如果未指定目录且待比对区已有 2 个目录，会提示直接开始对比。
  - **智能对比**:
    - **结构检查**: 如果两个目录结构差异过大，会提示用户是否继续。
    - **originSource 优化**:
      - **主对比**: 针对 `originSource` 下的组件仓库，优先检查 Git Tag 和状态。如果版本一致且无未提交修改，则视为相同，跳过耗时的文件内容对比，并显示版本摘要。
      - **子目录对比 (`-os`)**: 对 `originSource` 下指定的子目录进行深度递归内容对比。
    - **可视化输出**:
      - 使用加粗颜色和 `+`/`-` 前缀清晰区分新增（绿色）、删除（红色）和修改。
      - 支持行内差异高亮显示。
      - 自动忽略 `.git`、隐藏文件和 `.gitignore` 中列出的文件（除非强制对比 `originSource`）。
  - **报告生成**:
    - 对比结果会自动保存为 **Markdown 文件** (`.md`)，路径通常为 `~/.config/bsr/reports/diff_YYYYMMDD_HHMMSS.md`。
    - 使用 `diff` 代码块实现语法高亮。
    - 任务完成后，提供选项（Open / Cancel）直接在 VS Code 中打开生成的报告文件（默认为 Cancel）。
